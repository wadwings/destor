...
GHashTable * fp_to_superfps, * superfps_to_chunk;
...
void init_superfps_to_chunk()
{
	superfps_to_chunk = g_hash_table_new_full(g_int64_hash,
...
//  index_buffer_post_compress.chunk_num = 0;
  ...
  if (destor.index_specific != INDEX_SPECIFIC_NO)
  {
    ...
    case INDEX_SPECIFIC_BLOCK_LOCALITY_CACHING:
    {
      ...
      destor.index_segment_prefech = destor.index_segment_prefech > 1 ? destor.index_segment_prefech : 16;
      break;
    }
    case INDEX_SPECIFIC_SAMPLED:
    {
      ...
      destor.index_sampling_method[0] = INDEX_SAMPLING_UNIFORM;
      destor.index_sampling_method[1] = destor.index_sampling_method[1] > 1 ? destor.index_sampling_method[1] : 128;
      break;
    }
...
static void resemble_detection(struct segment *s)
{
  ...
  for (; iter != end; iter = g_sequence_iter_next(iter))
  {
    ...
    struct subchunks *subchunks;
#ifdef FINESSE_ENABLE
    subchunks = index_sampling_finesse(c);
#else
    subchunks = index_sampling_super_features(c);
#endif
    /*
...
//				struct fp_id_elem * em = g_hash_table_lookup(superfps_to_chunk, &subchunks->super_features[i]);
//				if(em){
//					c->id = em->id;
//					g_queue_push_head(em->ids, &c->id);
//					kvstore_update_fp_to_fp(c->fp, em->fp);
...
//		}
    ...
    if (!CHECK_CHUNK(c, CHUNK_DELTA_COMPRESS))
    {
      ...
      for (int i = 0; i < SUPER_FINGERPRINT_SIZE; i++)
      {
        post_compress_entry *entries = kvstore_lookup_post_compress((char *)&subchunks->super_features[i]);
        if (entries)
        {
        ...
        else
        {
          ...
          VERBOSE("Post Compression phase: non-existing fingerprint");
        }
      }
    }
...
//		if(!CHECK_CHUNK(c, CHUNK_DELTA_COMPRESS)){
//			for(int i = 0; i < SUPER_FINGERPRINT_SIZE; i++){
//				struct fp_id_elem * em = malloc(sizeof(struct fp_id_elem));
//				em->id = c->id;
//				em->ids = g_queue_new();
//				memcpy(&em->fp, &c->fp, sizeof(fingerprint));
//				memcpy(&em->sfp, &subchunks->super_features[i], sizeof(fingerprint));
//				g_hash_table_replace(superfps_to_chunk, &em->sfp, em);
//				test_index++;
//				NOTICE("add 1, now is %d",test_index);
//			}
//		}
...
static void index_lookup_base(struct segment *s)
{
  ...
  for (; iter != end; iter = g_sequence_iter_next(iter))
  {
    ...
    if (storage_buffer.container_buffer && lookup_fingerprint_in_container(storage_buffer.container_buffer, &c->fp))
    {
...
int index_lookup(struct segment *s)
{
  ...
  TIMER_DECLARE(1);
  TIMER_BEGIN(1);
  if (destor.index_category[1] == INDEX_CATEGORY_LOGICAL_LOCALITY && destor.index_segment_selection_method[0] != INDEX_SEGMENT_SELECT_BASE)
  {
    /* Similarity-based */
...
void index_update_post_compress(struct _GSequence * chunks, int64_t id)
{
	...
	for (; iter != end; iter = g_sequence_iter_next(iter)){
		...
		for(int i = 0; i < SUPER_FINGERPRINT_SIZE; i++){
			...
			index_overhead.update_requests++;
			kvstore_update_post_compress(*sfp, id, c->fp);
//			if(!CHECK_CHUNK(c, CHUNK_DELTA_COMPRESS)){
//				struct fp_id_elem * em = g_hash_table_lookup(superfps_to_chunk, sfp);
//				if(em){
//					while(!g_queue_is_empty(em->ids)){
//						int64_t * t = g_queue_pop_tail(em->ids);
////						*t = id;
//					}
//					g_queue_free(em->ids);
//					g_hash_table_remove(superfps_to_chunk, sfp);
//					test_index--;
//					NOTICE("remove 1, now is %d",test_index);
//				}
//			}
		}
	}
...
void index_check_buffer(struct segment *s)
{
  ...
  for (; iter != end; iter = g_sequence_iter_next(iter))
  {
    ...
    if ((CHECK_CHUNK(c, CHUNK_DUPLICATE) && c->id == TEMPORARY_ID) || CHECK_CHUNK(c, CHUNK_OUT_OF_ORDER) || CHECK_CHUNK(c, CHUNK_SPARSE))
    {
      /*
