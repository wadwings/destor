...
static void *filter_thread(void *arg)
{
  ...
  while (1)
  {
    ...
    for (; iter != end; iter = g_sequence_iter_next(iter))
    {
      ...
      if (!CHECK_CHUNK(c, CHUNK_DUPLICATE) || (!CHECK_CHUNK(c, CHUNK_REWRITE_DENIED) && (CHECK_CHUNK(c, CHUNK_SPARSE) || (enable_rewrite && CHECK_CHUNK(c, CHUNK_OUT_OF_ORDER) && !CHECK_CHUNK(c, CHUNK_IN_CACHE)))))
      {
        /*
        ...
        if (add_chunk_to_container(storage_buffer.container_buffer, c))
        {
          ...
          wc->id = c->id;
          if (!CHECK_CHUNK(c, CHUNK_DUPLICATE) && !CHECK_CHUNK(c, CHUNK_DELTA_COMPRESS))
          {
        ...
        else
        {
          ...
          assert(destor.index_category[0] != INDEX_CATEGORY_EXACT || destor.rewrite_algorithm[0] != REWRITE_NO);
        }
      }
      else
      {
