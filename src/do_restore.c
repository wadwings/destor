  ...
  while ((c = sync_queue_pop(restore_recipe_queue)))
  {
    ...
    else if (CHECK_CHUNK(c, CHUNK_DELTA_COMPRESS))
    {
      ...
      memcpy(fp, kvstore_lookup_fp_to_fp(c->fp), sizeof(fingerprint));
			int64_t * id = kvstore_lookup(fp);

      con = lru_cache_lookup(cache, &fp);
      if (!con)
      {
      ...
      struct chunk *rc_t;
      rc_t = get_chunk_in_container(con, &fp);

      assert(rc);
      assert(rc_t);

      char *t = (char *)malloc(destor.chunk_max_size);
      rc->size = xdelta3_decode(rc_t->data, rc->data, rc_t->size, rc->size, t);
...
static void *read_recipe_thread(void *arg)
{
  ...
  for (i = 0; i < jcr.bv->number_of_files; i++)
  {
    ...
    for (j = 0; j < r->chunknum; j++)
    {
      ...
      if (kvstore_lookup_fp_to_fp(cp->fp))
      {
